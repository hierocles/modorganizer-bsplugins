...
stability and group-management work, updated to build and run against current MO2.
...
No public MO2 release ships Qt 6.11+ yet, though beta releases are available in the MO2 Discord.. For MO2 2.5.2, use the `0.1.5.0b` release
([plugindefinition.json](plugindefinition.json) has both).


## Building

Two ways: standalone against a `mob`/`modorganizer_super` tree, or nested inside
...
- A built `mob` or `modorganizer_super` tree, with at least `cmake_common`, `uibase`, `bsatk`,
...
`DEPENDENCIES_DIR` is the tree's root: `cmake_common` is expected under
`<DEPENDENCIES_DIR>/modorganizer_super/cmake_common`, `.../build/cmake_common`, or
