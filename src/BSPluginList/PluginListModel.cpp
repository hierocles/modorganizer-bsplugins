...
{
  const auto plugin = m_Plugins->getPlugin(index.row());
  if (plugin && plugin->isMasterOfSelectedPlugin()) {
    return Settings::instance()->masterColor();
  }
...
QVariant PluginListModel::tooltipData(const QModelIndex& index) const
{
  ...
  case COL_NAME: {
    ...
    if (plugin->formVersion() != 0) {
      ...
      toolTip +=
          "<br><b>" + tr("Form Version") + "</b>: " + QString::number(plugin->formVersion());
    }
